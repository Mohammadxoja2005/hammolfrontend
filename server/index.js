...
app.get("/api/category/:name", (req,res) => {
    ...
    const found = products.products.filter(value => value.category == name); 
...
app.get("/getproduct/:name", (req,res) => {
  ...
  const found = products.products.filter(value => {
    return value.title.toLowerCase().includes(name) || value.brand.toLowerCase().includes(name);
  })  
